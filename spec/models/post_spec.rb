...
RSpec.describe Post, type: :model do
	...
	context "Scopes" do 
		...
		it "by_branch scope gets post by particular branch" do
			category = create(:category)
			create(:post, category_id: category.id)
			create_list(:post, 10)
	...
	context "Validatin" do
		...
		it "it not valid with title, shorter than 5 characters" do
			post.title = "a" * 4
			expect(post).not_to be_valid
		end

		it "it not valid with title, longer than 255 characters" do
			post.title = "a" * 256
			expect(post).not_to be_valid
		end
